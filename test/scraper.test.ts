...
const { scrapeCustomGPTs, fuzzyScore } = await import("../src/content.ts");

interface ExpectedGPT {
  name: string;
  url: string;
  image?: string;
}
...
describe("ChatGPT Switcher Extension", () => {
  describe("scrapeCustomGPTs", () => {
    ...
    it(`should correctly scrape GPTs from ${latestFixture} snapshot`, () => {
      ...
      const expected: ExpectedGPT[] = JSON.parse(readFileSync(expectedPath, "utf-8"));
